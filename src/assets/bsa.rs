...
impl Bsa {
    pub(crate) fn new(path: &str) -> Result<Self> {
        ...
        let magic = four_u8_to_le_u32(&header, 0).with_context(|| "Failed to read magic number from header")?;
        ...
        let meta_size = four_u8_to_le_u32(&header, 4).with_context(|| "Failed to read meta size from header")?;
        let num_files = four_u8_to_le_u32(&header, 8).with_context(|| "Failed to read files quantity from header")?;
        let meta_size_usize =
            usize::try_from(meta_size).with_context(|| format!("Bug: failed to cast \"{meta_size}\"(meta_size, u32) to usize"))?;
        let num_files_usize =
            usize::try_from(num_files).with_context(|| format!("Bug: failed to cast \"{num_files}\"(num_files, u32) to usize"))?;
        ...
        let names_string = from_utf8(
            meta.get(
                num_files_usize
                    .checked_mul(12_usize)
                    .with_context(|| format!("Bug: overflow multiplying 12 by num_files_usize = \"{num_files_usize}\""))?..,
            )
        ...
        let data_offset = u64::try_from(data_offset_usize)
            .with_context(|| format!("Bug: failed to cast \"{data_offset_usize}\"(data_offset_usize, usize) to u64"))?;
        Ok(Self {
            path: path.to_owned(),
            names,
            data_offset,
    ...
    pub(crate) fn get_file_by_index(&self, file_index: usize) -> Result<Vec<u8>> {
        let index = file_index
            .checked_mul(8)
            .with_context(|| format!("Bug: overflow multiplying 8 by file_index = \"{file_index}\""))?;
        let file_size = four_u8_to_le_u32(&self.meta, index).with_context(|| "Failed to read file size from meta")?;
...
fn read_n_u8(file: &File, take: u32) -> Result<Vec<u8>> {
    ...
    let take_usize = usize::try_from(take).with_context(|| format!("Bug: failed to cast \"{take}\"(take, u32) to usize"))?;
    ...
    match handle.read_to_end(&mut buffer) {
        Ok(num) => {
            if num == take_usize {
                ...
                Err(anyhow!("Failed to read bytes into buffer: expected {take:?}, got {num:?}"))
            }
        }
