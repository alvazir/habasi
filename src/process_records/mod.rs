use crate::{Cfg, Helper, Log, Out, StatsUpdateKind};
    ...
    for (count, record) in plugin.objects.into_iter().enumerate() {
        match count {
            0 => {
                ...
                process_header(header, out, h)?;
            }
            _ => match record {
                TES3Object::Dialogue(dial) => process_dial(dial, out, h),
                TES3Object::DialogueInfo(info) => process_info(info, out, h)?,
                _ => {
